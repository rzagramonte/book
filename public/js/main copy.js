...
images.forEach((img) => {
  ...
  const src1x = baseUrl;
  const src2x = `${urlParts[0]}/upload/w_800,dpr_2.0/${urlParts[1]}`;
  ...
  const applyOrientationAndScroll = () => {
    img.classList.add(img.naturalWidth > img.naturalHeight ? "landscape" : "portrait");
    chatBox?.scrollTo(0, chatBox.scrollHeight);
  };

  if (img.complete) {
    applyOrientationAndScroll();
  } else {
    img.addEventListener("load", applyOrientationAndScroll);
  }
});
